...
class DataTrainingArguments:
    ...
    predict_langs: Optional[str] = field(default='en', metadata={"help": "The language of the data."})
...
def main():
    ...
    if data_args.task == 'xcopa':
        datasets = load_dataset("xcopa", data_args.train_lang)
    elif data_args.task == 'siqa':
        datasets = load_dataset("lighteval/siqa")
    else:
    ...
    if data_args.predict_langs is not None and training_args.do_predict:
      ...
      for lang in predict_langs:
        ...
        datasets = load_dataset("xcopa", lang)
        tokenized_datasets = datasets.map(
            preprocess_function,
            batched=True,
            num_proc=data_args.preprocessing_num_workers,
