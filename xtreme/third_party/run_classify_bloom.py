...
""" Finetuning multi-lingual models on XNLI/PAWSX (BLOOM)."""
...
from transformers import BLOOM_PRETRAINED_CONFIG_ARCHIVE_MAP
from transformers import BitsAndBytesConfig
from transformers import BloomConfig, BloomTokenizerFast, BloomForSequenceClassification
...
ALL_MODELS = tuple(BLOOM_PRETRAINED_CONFIG_ARCHIVE_MAP.keys())
...
def train(args, train_dataset, model, tokenizer, lang2id=None):
  ...
  for _ in train_iterator:
    ...
    for step, batch in enumerate(epoch_iterator):
      ...
      if args.model_type != "distilbert" and args.model_type != 'bloom':
...
def main():
  ...
  parser.add_argument("--apply_qlora", action="store_true", help="Apply QLoRA adapter to model!")
  ...
  if args.local_rank == -1 or args.no_cuda:
    ...
    args.n_gpu = 1 if args.cache_dir else torch.cuda.device_count() 
  ...
  # deal with model path 'bigscience/bloom...' or 'bigscience/bloom-xxx-intermediate-global_stepxxx'
  tokenizer_path = 'bigscience/' + args.model_name_or_path
  model_path  = 'bigscience/' + args.model_name_or_path
  if 'intermediate' in model_path: # intermediate bloom
    name_and_step_list = model_path.split('-intermediate-')
    tokenizer_path = name_and_step_list[0]
    model_path = name_and_step_list[0] + '-intermediate'
    args_dict['revision'] = name_and_step_list[1]
    logger.info("Revision: {}".format(name_and_step_list[1]))
  ...
  tokenizer = tokenizer_class.from_pretrained(
    args.tokenizer_name if args.tokenizer_name else tokenizer_path,
  ...
  if args.apply_qlora:
    ...
    args_dict['quantization_config'] = nf4_config
    args_dict['device_map'] = "auto"
  ...
  if args.do_train:
    ...
    if not args.apply_qlora:
      model.to(args.device)
    else:
      # preprocess petf model to be trained
  ...
  if args.do_eval and args.local_rank in [-1, 0]:
    tokenizer = tokenizer_class.from_pretrained(args.output_dir, do_lower_case=args.do_lower_case, cache_dir=args.cache_dir if args.cache_dir else None)
    ...
    for checkpoint in checkpoints:
      ...
      if args.apply_qlora:
        peft_config = PeftConfig.from_pretrained(best_checkpoint)
        peft_config.inference_mode = True
        model = model_class.from_pretrained(peft_config.base_model_name_or_path, cache_dir=args.cache_dir if args.cache_dir else None, **args_dict)
  ...
  if args.do_predict and args.local_rank in [-1, 0]:
    tokenizer = tokenizer_class.from_pretrained(tokenizer_path if args.model_name_or_path else best_checkpoint, do_lower_case=args.do_lower_case, cache_dir=args.cache_dir if args.cache_dir else None)
    if args.apply_qlora:
      peft_config = PeftConfig.from_pretrained(best_checkpoint)
      peft_config.inference_mode = True
      model = model_class.from_pretrained(peft_config.base_model_name_or_path, cache_dir=args.cache_dir if args.cache_dir else None, **args_dict)
  ...
  if args.do_predict_dev:
    tokenizer = tokenizer_class.from_pretrained(tokenizer_path if args.model_name_or_path else best_checkpoint, do_lower_case=args.do_lower_case)
