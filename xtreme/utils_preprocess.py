...
import argparse
from transformers import BertTokenizer, XLMTokenizer, XLMRobertaTokenizer, BloomTokenizerFast
import os
      ...
      for l in lines:
        ...
        if len(items) == 2:
          ...
          if idx != -1:
            ...
            # =================where the test tag is deleted======================
            # Erika: For convenient use, disable the deleted tag in test file.
            # if 'test' in infile:
            #   fout.write(f'{token}\n')
            # else:
            #   fout.write(f'{token}\t{label}\n')
...
def udpos_tokenize_preprocess(args):
  ...
  model_type = args.model_type   # bloom
  tokenizer = TOKENIZERS[model_type].from_pretrained(args.model_name_or_path,   
                                                do_lower_case=args.do_lower_case,
                                                cache_dir=args.cache_dir if args.cache_dir else None)
  for lang in args.languages.split(','):
    ...
    for file in files:
      ...
      if args.model_type == 'bloom':
        ...
        if args.bloom_intermediate:
          model_name = args.cached_model_name_or_path.split('/')[1]
              
      infile = os.path.join(args.data_dir, "{}-{}.tsv".format(file, lang))
      outfile = os.path.join(out_dir, "{}.{}".format(file, model_name))
      idxfile = os.path.join(out_dir, "{}.{}.idx".format(file,  model_name))
      if os.path.exists(outfile) and os.path.exists(idxfile):
        print(f'{outfile} and {idxfile} exist')
      else:
...
def tatoeba_preprocess(args):
  # Erika: this function still enables the test label removing
  lang3_dict = {
...
def mewslix_preprocess(args):
  ...
  preprocess_split('test', remove_labels=False)
...
if __name__ == "__main__":
  ...
  # deal with the bloom path problem
  if args.model_type == 'bloom':
    args.model_name_or_path = 'bigscience/' + args.model_name_or_path
    args.bloom_intermediate = False
    if 'intermediate' in args.model_name_or_path:  # intermediate bloom
      # reduce model name to the original
      args.bloom_intermediate = True
      args.cached_model_name_or_path = args.model_name_or_path
      args.model_name_or_path = args.model_name_or_path.split('-intermediate-')[0]
